...
isTruthy(-1);
isTruthy({});
isTruthy({nome: "Gustavo", profissao: "Luber"});
isTruthy([]);
isTruthy(["Gustavo", "Luby"]);
isTruthy(function(){});
isTruthy([{}]);
...
carro.adicionarPessoas = function(pessoas) {
    ...
    if(assentosDisponiveis === 0 && pessoas > 0) {
        return `O carro já está lotado!`;
    } else if(pessoas > assentosDisponiveis) {
        return assentosDisponiveis === 1 ? `Só cabe mais 1 pessoa!`
        : `Só cabem mais ${assentosDisponiveis} pessoas!`;
    } else {
...
carro.obterMarcaModelo(); // Este carro é um Lamborghini Gallardo
